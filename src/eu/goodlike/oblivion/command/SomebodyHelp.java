...
public final class SomebodyHelp extends BaseCommand {
  ...
  protected void performTask() {
    ...
    Write.line("'parse lenient' -> allows duplicate effect types for items and spells both from file and input");
    Write.line("'parse mixed' -> allows duplicate effect types for items and spells only from file (default behavior)");
    Write.line("'parse strict' -> forbids duplicate effect types for items and spells");
    ...
    Write.line("'refresh' -> clears all queued hits, updates the enemy (e.g. if player level was changed)");
    Write.line("'reload' -> reloads caches");
    Write.line("Note: if you change the prepared files, they will not be re-loaded. Use 'restart' for that.");
    Write.line("'reset' -> reloads absolutely everything (almost equivalent to restarting the application)");
    Write.line("Note: if you change the settings or prepared files, they will not be re-loaded. Use 'restart' for that.");
    Write.line("'restart' -> reloads absolutely everything (equivalent to restarting the application)");
